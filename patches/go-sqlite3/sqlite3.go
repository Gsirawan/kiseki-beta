...
func (d *SQLiteDriver) Open(dsn string) (driver.Conn, error) {
	...
	// SQLCipher encryption key (applied as first PRAGMA after open)
	...
	if pos >= 1 {
		...
		// SQLCipher encryption key (_key)
		//
		// If provided, PRAGMA key will be issued as the very first statement
		// after sqlite3_open_v2, before any other PRAGMA. This is required by
		// SQLCipher — any statement before PRAGMA key on an encrypted database
		// causes "file is not a database".
	...
	// SQLCipher encryption key (_key)
	// MUST be the very first PRAGMA after sqlite3_open_v2.
	// If the database is encrypted and the wrong key (or no key) is provided,
	// all subsequent operations will fail with "file is not a database".
	if encryptionKey != "" {
		escapedKey := strings.ReplaceAll(encryptionKey, "'", "''")
		if err := exec(fmt.Sprintf("PRAGMA key = '%s';", escapedKey)); err != nil {
			C.sqlite3_close_v2(db)
			return nil, err
		}
	}
