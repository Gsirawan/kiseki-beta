...
> **Gist** gives you the gist. A lightweight subagent pattern that searches Kiseki on behalf of your primary AI session — keeping raw memory chunks out of your expensive model's context window.

---

## What Is Gist?

When your AI calls `kiseki_search` directly, the raw memory chunks land in its context window. A thorough search across multiple queries can easily consume 5,000–10,000+ tokens — in your primary model's context, at your primary model's cost.
...
|---|---|---|
| Primary model calls `kiseki_search` directly | High — raw chunks in expensive context | Good |
...
**When to skip Gist and call `kiseki_search` directly:**
...
### 1. Configure Kiseki MCP (if not already done)

Add Kiseki to your MCP config (`.mcp.json` or `opencode.json`):
...
```json
{
  ...
  "agents": {
    "gist": {
      ...
      "instructions": "You are Gist — a focused memory retrieval agent for Kiseki. Your only job: search Kiseki memory, read the results carefully, and return a clean structured summary. See agents/gist.md for full instructions."
    }
  }
}
```
...
Kiseki is persistent memory. Gist is the retrieval agent.

**The rule: delegate memory searches to Gist.**

Instead of calling `kiseki_search` directly, use:
```
task(subagent_type="gist", prompt="Search Kiseki for [topic]. Return all relevant chunks and a summary.")
```

Only call `kiseki_search` directly for quick single-query lookups where token cost doesn't matter.
```
...
task(
    subagent_type="gist",
    prompt="Search Kiseki for our decision to use event sourcing. Why did we choose it over CRUD?"
)
```
...
task(
    subagent_type="gist",
    prompt="Use kiseki_history to get the full timeline of mentions of 'PaymentService'. What changed over time?"
)
```
...
task(
    subagent_type="gist",
    prompt="Search Kiseki for everything related to our authentication system: JWT setup, session management, the security incident in Q3, and any open questions. Run multiple searches and give me a complete picture."
)
```
...
task(
    subagent_type="gist",
    prompt="Search Kiseki for database migration discussions from before 2026-01-15. What was the state of the migration at that point?"
)
```
...
| Quick single lookup, cheap model | Direct `kiseki_search` |
| You want to inspect raw chunks yourself | Direct `kiseki_search` |
| Debugging what's in Kiseki | Direct `kiseki_search` |
...
- 3 × `kiseki_search` calls × ~10 chunks each × ~300 tokens/chunk = **~9,000 tokens**
