...
func TestHistory(t *testing.T) {
	...
	results, err := History(dbConn, "Go", 10)
	if err != nil {
...
func TestHistoryNullDates(t *testing.T) {
	...
	results, err := History(dbConn, "memory", 10)
	if err != nil {
		t.Fatalf("History failed: %v", err)
	}
...
func TestHistoryAliases(t *testing.T) {
	...
	t.Setenv("KISEKI_ALIASES", "alice=alice,bob,roberto")
	LoadAliasesFromEnv()

	// Searching "Alice" should find Alice, Bob, and Roberto chunks (all aliases)
	results, err := History(dbConn, "Alice", 10)
	if err != nil {
		t.Fatalf("History failed: %v", err)
	}
	if len(results) != 3 {
	...
	results, err = History(dbConn, "Charlie", 10)
	if err != nil {
		t.Fatalf("History failed: %v", err)
	}
	if len(results) != 1 {
...
func TestHistoryLimit(t *testing.T) {
	...
	// Test explicit limit
	results, err := History(dbConn, "test", 5)
	if err != nil {
		t.Fatalf("History failed: %v", err)
	}
	...
	results, err = History(dbConn, "test", 0)
	if err != nil {
		t.Fatalf("History failed: %v", err)
	}
	...
	results, err = History(dbConn, "test", -1)
	if err != nil {
		t.Fatalf("History failed: %v", err)
	}
