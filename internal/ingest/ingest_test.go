...
// testEmbedResponse mirrors ollama's internal embedResponse for test mocking
...
func TestIngestFile(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		embedding[0] = 0.42
		resp := testEmbedResponse{Embeddings: [][]float64{embedding}}
	...
	result, err := IngestFile(dbConn, client, filePath, "2024-01-01T00:00:00Z")
	if err != nil {
	...
	if err := dbConn.QueryRow("SELECT source_file, valid_at, ingested_at FROM chunks LIMIT 1").Scan(&storedSource, &storedValid, &storedIngested); err != nil {
...
func TestIngestFileSectionDates(t *testing.T) {
	...
	rows, err := dbConn.Query("SELECT section_title, valid_at FROM chunks ORDER BY section_sequence, chunk_sequence")
	if err != nil {
