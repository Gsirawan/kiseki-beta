...
func IngestFile(db *sql.DB, embedder ollama.Embedder, filePath string, validAt string, formats ...string) (IngestResult, error) {
	...
	return IngestContent(db, embedder, string(data), filePath, validAt, format)
}

// IngestContent parses content in the given format and ingests it into the database.
// sourceName is stored as the source_file for each chunk (typically a file path or label).
func IngestContent(db *sql.DB, embedder ollama.Embedder, content string, sourceName string, validAt string, format string) (IngestResult, error) {
	sections := ParseContent(content, sourceName, format)
	...
	for _, section := range sections {
		...
		for _, chunk := range chunks {
			...
			embedding, err := embedder.Embed(ctx, normalizedText)
			if err != nil {
				return IngestResult{}, err
			}
	...
	db.Exec(`DELETE FROM vec_chunks WHERE chunk_id IN (SELECT id FROM chunks WHERE source_file = ?)`, sourceName)
	delResult, err := db.Exec("DELETE FROM chunks WHERE source_file = ?", sourceName)
	if err != nil {
		return IngestResult{}, err
	}
